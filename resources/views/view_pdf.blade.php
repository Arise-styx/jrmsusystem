...
<body>
    {{-- <div class="container-fluid">
        <h2 class="text-center"> List of Purchased Materials </h2>
        <h2 class="display-5 fw-bold">{{ $monthname }} {{ $year }}</h2>
        <table class="table table-striped table-success table-bordered table-hover">
            ...
            <tbody class="table-group-divider">
                @foreach ($purchasedmaterials as $material)
                    <tr>
                        <!-- <th scope="row">1</th> -->
                        <td>{{ $material->employee_name }}</td>
                        <td>{{ $material->material_code }}</td>
                        <td>{{ $material->material_description }}</td>
                        <td>{{ $material->supplier }}</td>
                        <td>{{ number_format($material->amount, 2, '.', ',') }}</td>
                        <td>{{ $material->date_of_purchase }}</td>
                        <td>{{ $material->issued_to }}</td>
    ...
    <br>
    <footer>**Report generated by <strong><cite title="Source Title">{{ $user->name }}</cite></strong></footer> --}}
            ...
            <table cellpadding="1" cellspacing="0" border="0">
                <tbody>
                    @foreach ($purchasedmaterials as $material)
                        <tr>
                            <!-- <th scope="row">1</th> -->
                            <td style="text-transform: uppercase;">{{ $material->employee_name }}</td>
                            <td>{{ $material->material_code }}</td>
                            <td>{{ $material->material_description }}</td>
                            <td>{{ $material->supplier }}</td>
                            <td>{{ number_format($material->amount, 2, '.', ',') }}</td>
